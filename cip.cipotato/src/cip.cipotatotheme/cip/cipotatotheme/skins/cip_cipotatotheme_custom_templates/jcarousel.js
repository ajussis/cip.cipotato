...
/* - jcarousel.js - */
// http://www.brasil.gov.br/portal_javascripts/jcarousel.js?original=1
(function($j){$j.fn.jcarousel=function(o){return this.each(function(){new $jc(this,o)})};var defaults={vertical:false,start:1,offset:1,size:null,scroll:3,visible:null,animation:'normal',easing:'swing',auto:0,wrap:null,initCallback:null,reloadCallback:null,itemLoadCallback:null,itemFirstInCallback:null,itemFirstOutCallback:null,itemLastInCallback:null,itemLastOutCallback:null,itemVisibleInCallback:null,itemVisibleOutCallback:null,buttonNextHTML:'<div></div>',buttonPrevHTML:'<div></div>',buttonNextEvent:'click',buttonPrevEvent:'click',buttonNextCallback:null,buttonPrevCallback:null};$j.jcarousel=function(e,o){this.options=$j.extend({},defaults,o||{});this.locked=false;this.container=null;this.clip=null;this.list=null;this.buttonNext=null;this.buttonPrev=null;this.wh=!this.options.vertical?'width':'height';this.lt=!this.options.vertical?'left':'top';var skin='',split=e.className.split(' ');for(var i=0;i<split.length;i++){if(split[i].indexOf('jcarousel-skin')!=-1){$j(e).removeClass(split[i]);var skin=split[i];break}}
if(e.nodeName=='UL'||e.nodeName=='OL'){this.list=$j(e);this.container=this.list.parent();if(this.container.hasClass('jcarousel-clip')){if(!this.container.parent().hasClass('jcarousel-container'))
...
this.container=this.list.wrap('<div></div>').parent()} else{this.container=$j(e);this.list=$j(e).find('>ul,>ol,div>ul,div>ol')}
...
this.clip=this.list.wrap('<div></div>').parent();this.buttonPrev=$j('.jcarousel-prev',this.container);if(this.buttonPrev.size()==0&&this.options.buttonPrevHTML!=null)
this.buttonPrev=this.clip.before(this.options.buttonPrevHTML).prev();this.buttonPrev.addClass(this.className('jcarousel-prev'));this.buttonNext=$j('.jcarousel-next',this.container);if(this.buttonNext.size()==0&&this.options.buttonNextHTML!=null)
...
this.options.initCallback(this,'init');if($j.browser.safari){this.buttons(false,false);$j(window).bind('load', function(){self.setup()})} else
this.setup()};var $jc=$j.jcarousel;$jc.fn=$jc.prototype={jcarousel:'0.2.3'};$jc.fn.extend=$jc.extend=$j.extend;$jc.fn.extend({setup: function(){this.first=null;this.last=null;this.prevFirst=null;this.prevLast=null;this.animating=false;this.timer=null;this.tail=null;this.inTail=false;if(this.locked)
return;this.list.css(this.lt,this.pos(this.options.offset)+'px');var p=this.pos(this.options.start);this.prevFirst=this.prevLast=null;this.animate(p,false);$j(window).unbind('resize',this.funcResize).bind('resize',this.funcResize)},reset: function(){this.list.empty();this.list.css(this.lt,'0px');this.list.css(this.wh,'10px');if(this.options.initCallback!=null)
...
this.options.reloadCallback(this);if(this.options.visible!=null){var self=this;var di=Math.ceil(this.clipping()/this.options.visible),wh=0,lt=0;$j('li',this.list).each(function(i){wh+=self.dimension(this,di);if(i+1<self.first)
...
return false}
return true},get: function(i){return $j('.jcarousel-item-'+i,this.list)},add: function(i,s){var e=this.get(i),old=0,add=0;if(e.length==0){var c,e=this.create(i),j=$jc.intval(i);while(c=this.get(--j)){if(j<=0||c.length){j<=0?this.list.prepend(e):c.after(e);break}}} else
...
return;var callback=typeof this.options[cb]=='object'?this.options[cb][evt]:this.options[cb];if(!$j.isFunction(callback))
...
if(i!==null&&!(i>=i3&&i<=i4))
this.get(i).each(function(){callback(self,this,i,state,evt)})}},create: function(i){return this.format('<li></li>',i)},format: function(e,i){var $e=$j(e).addClass(this.className('jcarousel-item')).addClass(this.className('jcarousel-item-'+i));$e.attr('jcarouselindex',i);return $e},className: function(c){return c+' '+c+(!this.options.vertical?'-horizontal':'-vertical')},dimension: function(e,d){var el=e.jquery!=undefined?e[0]:e;var old=!this.options.vertical?el.offsetWidth+$jc.margin(el,'marginLeft')+$jc.margin(el,'marginRight'):el.offsetHeight+$jc.margin(el,'marginTop')+$jc.margin(el,'marginBottom');if(d==undefined||old==d)
return old;var w=!this.options.vertical?d-$jc.margin(el,'marginLeft')-$jc.margin(el,'marginRight'):d-$jc.margin(el,'marginTop')-$jc.margin(el,'marginBottom');$j(el).css(this.wh,w+'px');return this.dimension(el)},clipping: function(){return!this.options.vertical?this.clip[0].offsetWidth-$jc.intval(this.clip.css('borderLeftWidth'))-$jc.intval(this.clip.css('borderRightWidth')):this.clip[0].offsetHeight-$jc.intval(this.clip.css('borderTopWidth'))-$jc.intval(this.clip.css('borderBottomWidth'))},index: function(i,s){if(s==undefined)
s=this.options.size;return Math.round((((i-1)/ s) - Math.floor((i-1) / s)) * s)+1}});$jc.extend({defaults: function(d){return $j.extend(defaults,d||{})},margin: function(e,p){if(!e)
return 0;var el=e.jquery!=undefined?e[0]:e;if(p=='marginRight'&&$j.browser.safari){var old={'display':'block','float':'none','width':'auto'},oWidth,oWidth2;$j.swap(el,old, function(){oWidth=el.offsetWidth});old['marginRight']=0;$j.swap(el,old, function(){oWidth2=el.offsetWidth});return oWidth2-oWidth}
return $jc.intval($j.css(el,p))},intval: function(v){v=parseInt(v);return isNaN(v)?0:v}})})(jQuery);
